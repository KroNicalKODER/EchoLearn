...
app.post("/tgtojson", async (req, res) => {
  ...
  try {
    ...
    const phones = json.data.tiers
      .find((tier) => tier.name === "phones")
      .intervals.map((interval) => interval.text);
    const phonesByMachine = json.data1.tiers
      .find((tier) => tier.name === "phones")
      .intervals.map((interval) => interval.text);
    console.log(phones);
    ...
    res
      .status(500)
      .json({ code: 500, error: "Internal Server Error", message: error });
  }
});
