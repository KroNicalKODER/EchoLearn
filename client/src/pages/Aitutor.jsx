...
const Aitutor = () => {
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        setTranscribedText(finalTranscript); // Set the final transcribed text
      };
  ...
  const getAIResponse = async (text) => {
    try {
      ...
      const response = await axios.post(`${config.flask_url}/generate-response`, JSON.stringify({ text }), {
        headers: {
      ...
      if (audioUrl) {
        const fullAudioUrl = `${config.flask_url}${audioUrl}`;  // Ensure correct path
  ...
  return (
    <div className="flex flex-col w-full justify-center items-center">
      ...
      {/* Audio player */}
      {responseAudio && (
        <audio controls id={responseAudio} crossOrigin="anonymous" className="w-full mt-6 rounded-lg border-none">
